...
def show_image_after_blur_analysis(img, mask) -> None:
    ...
    if SHOW_FIG_FLAG:
        cv2.imshow("output_blurred", np.hstack([output]))
    if SAVE_FIG_FLAG:
        cv2.imwrite('output_blurred.jpg',np.hstack([output]),[cv2.IMWRITE_JPEG_QUALITY, 70])
...
def main() -> None:
    ...
    show_image_after_blur_analysis(img, mask)
    # cv2.waitKey(0)
