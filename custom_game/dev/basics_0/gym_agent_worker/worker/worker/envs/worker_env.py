...
class WorkerEnvGym(gym.Env):
    ...
    def step(self, action):
        ...
        if changed_pos:
            self.agent_body.insert(0, list(self.agent_pos))
            self.agent_body.pop()
    ...
    @staticmethod
    def move(direction, agent_pos, frame_size_y, frame_size_x):
        changed_pos = False
            
        if direction == 'UP' and agent_pos[1] - 10 >= 0:
            agent_pos[1] -= 10
            changed_pos = True
        if direction == 'DOWN' and agent_pos[1] + 10 < frame_size_y:
            agent_pos[1] += 10
            changed_pos = True
        if direction == 'LEFT' and agent_pos[0] - 10 >= 0:
            agent_pos[0] -= 10
            changed_pos = True
        if direction == 'RIGHT' and agent_pos[0] + 10 < frame_size_x:
            agent_pos[0] += 10
            changed_pos = True
